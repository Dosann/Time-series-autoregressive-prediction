...
class DeterministicAutoregressivePredictor(Predictor):
    ...
    def __init__(self, solver):
        super(DeterministicAutoregressivePredictor, self).__init__(solver)
    
    def do_predict(self, X):
        predict = self._solver.predict(X)
    ...
    def multistep_predict(self, X, n_steps):
        ...
        if input_length != self._solver.input_length:
            print("input length of X ({}) equals not the model input length "
                  ...
                  .format(input_length, self._solver.input_length))
            input_length = self._solver.input_length
            X = X[:,-input_length:,:]
