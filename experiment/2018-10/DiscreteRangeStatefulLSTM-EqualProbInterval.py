...
from tsap.callbacks import Callback
import numpy as np
import pandas as pd
...
class DRSCallback(Callback):

    def __init__(self):
        super(DRSCallback, self).__init__()
        self.stage = 0

    @staticmethod
    def SaveModel(model):
        model.save('{}.{:0>4d}'.format(params['model_path'], model.current_epoch))
    ...
    def on_epoch_begin(self, batch, logs=None):
        solver_train._solver.reset_states()
        print("# LSTM states reset.")
    
    def on_stage_begin(self, model, logs=None):
        self.stage += 1
...
def draw_test_summary():
    ...
    pred_class = data_util.continue2discrete(pred_value, intervals)
    # draw figure
    f1 = DRSCallback.draw(true_value, pred_value)
    f2 = DRSCallback.draw(true_class, pred_class, prob)
    f1.savefig(params['model_path'] + 
...
if __name__ == '__main__':
    ...
    if not params['fit_generator']:
        ...
        valid_feeder2 = data_util.SeqDiscrtzdRandomChanlStatefulDF(
                    load_data(params['valid_path2']), 
                    params['batch_size'], params['batches_per_epoch'], 
                    params['input_length'], params['input_size'],
                    params['n_classes'], intervals)

    predictor_m = pp.MCMCPredictorStateful(params, intervals)
    predictor_d = dap.DetermDiscreteAGPredictorStateful(params, intervals)
    ...
    if not params['test']: # train phase
        ...
        test_feeder = data_util.SeqDiscrtzdRandomChanlStatefulDF(
